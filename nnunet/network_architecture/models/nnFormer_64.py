...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Conv3d(dim, dim * 2, kernel_size=2, stride=2) if dim*2<320 else nn.Conv3d(dim, 320, kernel_size=2, stride=2)
    ...
    def forward(self, x, S, H, W):
        ...
        x = self.reduction(x)
        x = x.permute(0, 2, 3, 4, 1).view(B, -1, 2 * C) if 2*C<320 else x.permute(0, 2, 3, 4, 1).view(B, -1, 320)
        return x
...
class encoder(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers)[::-1]:
            ...
            self.layers.append(layer)
        self.num_features = [int(embed_dim * 2 ** i) if int(embed_dim * 2 ** i)<320 else 320 for i in range(self.num_layers)]
...
class nnformer_64(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_64, self).__init__()
