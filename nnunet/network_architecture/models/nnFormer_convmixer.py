...
#Replace with convmixer

class ConvMixer(nn.Module):
    #use for resolution recover
    def __init__(self, in_channels, out_channels, drop = 0, kernel_size=3, stride=1, Conv3D=nn.Conv3d, norm_layer=nn.LayerNorm, act_layer=nn.GELU):
        super().__init__()
        self.depthwise = Conv3D(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=((kernel_size - 1) // 2), groups=in_channels)
        self.separable = Conv3D(in_channels, out_channels, kernel_size=1)
        self.act = act_layer()
...
class OperationBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, kernel_size = 5, stride = 1,
        ...
        self.operation_block = ConvMixer(dim, dim, drop = drop, kernel_size=kernel_size, stride=stride)
...
class nnformer_ConvMixer(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_ConvMixer, self).__init__()
