...
class SingleConv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, Conv3D=nn.Conv3d, norm_layer=nn.LayerNorm, act_layer=nn.GELU):
        super().__init__()
        self.conv = Conv3D(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=((kernel_size - 1) // 2))
        self.act = act_layer()
...
class patch_decoding(nn.Module):
    def __init__(self, dim, depths=[2, 2], drop_rate=0., drop_path_rate=0.2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.num_layers = len(depths)
        self.pos_drop = nn.Dropout(p=drop_rate)
        self.norm1 = norm_layer(dim)
        ...
        self.conv1_after_upsample1 = SingleConv3DBlock(in_channels=dim, out_channels=dim//2)

        self.upsample2 = nn.ConvTranspose3d(dim, dim//2, [2, 2, 2], [2, 2, 2])
        self.conv1_after_upsample2 = SingleConv3DBlock(in_channels=dim//2, out_channels=dim//2)
    ...
    def forward(self, skips, x):
        ...
        x = self.norm1(x)
        x = x.permute(0, 4, 1, 2, 3)
        x = self.conv1_after_upsample1(x)
        outputs.append(x)
        ...
        x = self.norm2(x)
        x = x.permute(0, 4, 1, 2, 3)
        x = self.conv1_after_upsample2(x)
        outputs.append(x)
...
class final_patch_expanding(nn.Module):
    def __init__(self, dim, num_class):
        super().__init__()
        self.ConvBlock = SingleConv3DBlock(dim, num_class)
...
class nnformer_v3(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_v3, self).__init__()
        ...
        self.model_down = SwinTransformer(pretrain_img_size=pretrain_img_size, window_size=window_size,
                                          ...
                                          num_heads=num_heads, in_chans=input_channels, out_indices=[0,1,2,3])
        ...
        self.patch_decoder = patch_decoding(embed_dim)
        self.final = []
        for i in range(2, 0, -1):
            self.final.append(final_patch_expanding(embed_dim //2, num_classes))
        self.final = nn.ModuleList(self.final)

    def forward(self, x):

        seg_outputs = []
        patch_embeddings, skips = self.model_down(x)
        neck = skips[-1]

        out = self.encoder(neck, skips)

        out1 = self.patch_decoder(patch_embeddings, out[-1])
        num_outputs = len(out1)
        for i in range(num_outputs):
            seg_outputs.append(self.final[0](out1[num_outputs-1-i]))
