...
class BasicLayer_up(nn.Module):
    """ Upsampling block for Swin-UNETR.
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
        patch_embed_x = x.view(-1, Ws, Wh, Ww, self.num_features[0]).permute(0, 4, 1, 2, 3).contiguous()
...
class SwinUNETR(SegmentationNetwork):
    ...
    def __init__(self, input_channels, num_classes, deep_supervision=True):
        ...
        patch_size = [2, 4, 4]
        window_size = 4
        self.first_conv = nn.Conv3d(1, self.num_classes, kernel_size=3, stride=1, padding=1)
        ...
        self.encoder = encoder(embed_dim=embed_dim, depths=depths[::-1][1:])
        self.final = []
    ...
    def forward(self, x):
        ...
        seg_outputs = []
        first_conv_x = self.first_conv(x)
        if self.norm is not None:
            Ws, Wh, Ww = first_conv_x.size(2), first_conv_x.size(3), first_conv_x.size(4)
            first_conv_x = first_conv_x.flatten(2).transpose(1, 2)
            first_conv_x = self.norm(first_conv_x)
            first_conv_x = first_conv_x.transpose(1, 2).view(-1, self.num_classes, Ws, Wh, Ww)
        ...
        for i in range(len(out)):
            if i==len(out)-1:
                seg_outputs.append(self.final[-(i + 1)](out[i]) + first_conv_x)
            else:
