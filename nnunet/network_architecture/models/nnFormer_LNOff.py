...
class project(nn.Module):
    def __init__(self, in_dim, out_dim, stride, padding, activate, last=False):
        super().__init__()
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96):
        ...
        self.proj1 = project(in_chans, embed_dim // 2, [1, 2, 2], 1, nn.GELU, False)
        self.proj2 = project(embed_dim // 2, embed_dim, [2, 2, 2], 1, nn.GELU, True)
...
class nnformer_LNOff(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_LNOff, self).__init__()
