#change Gelu to leaky relu
#remove shortcuts and dropPath
#Without residual connections within each block

#Without FFN and change layer normalization to Instance normalization
...
class SingleConv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, Conv3D=nn.Conv3d, norm_layer=nn.InstanceNorm3d, act_layer=nn.LeakyReLU):
        super().__init__()
        self.conv = Conv3D(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=((kernel_size - 1) // 2))
        self.act = act_layer(negative_slope= 1e-2, inplace=False)
...
class OperationBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, kernel_size = 5, stride = 1, drop=0.,
                 act_layer=nn.LeakyReLU, norm_layer=nn.InstanceNorm3d):
        super().__init__()
        self.dim = dim
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.InstanceNorm3d):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Conv3d(dim, out_channels=dim * 2 if dim*2<320 else 320, kernel_size=2, stride=2)
        self.norm = norm_layer(dim)
        self.act = nn.LeakyReLU(negative_slope= 1e-2, inplace=False)
    ...
    def forward(self, x, S, H, W):
        ...
        x = self.reduction(x)
        x = x.permute(0, 2, 3, 4, 1).view(B, -1, 2 * C if 2*C<320 else 320)
        return x
...
class Patch_Expanding(nn.Module):
    def __init__(self, dim, norm_layer=nn.InstanceNorm3d):
        ...
        self.dim = dim
        self.norm = norm_layer(dim)
        self.up = nn.ConvTranspose3d(dim, dim // 2, 2, 2) if dim<320 else nn.ConvTranspose3d(dim, 256, 2, 2)
    ...
    def forward(self, x, S, H, W):
        ...
        x = x.permute(0, 4, 1, 2, 3)
        x = self.up(x)
        x = x.permute(0, 2, 3, 4, 1).view(B, -1, C // 2) if C!=320 else x.permute(0, 2, 3, 4, 1).view(B, -1, 256)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
        ...
        self.Upsample = upsample(dim=2 * dim if 2*dim<320 else 320, norm_layer=norm_layer)
...
class project(nn.Module):
    def __init__(self, in_dim, out_dim, stride, padding, activate, norm, last=False):
        ...
        self.activate = activate(negative_slope= 1e-2, inplace=False)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.proj1 = project(in_chans, embed_dim // 2, [1, 2, 2], 1, nn.LeakyReLU, nn.InstanceNorm3d, False)
        self.proj2 = project(embed_dim // 2, embed_dim, [2, 2, 2], 1, nn.LeakyReLU, nn.InstanceNorm3d, True)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                dim=int(embed_dim * 2 ** i_layer) if int(embed_dim*2**i_layer)<320 else 320,
                input_resolution=(
        ...
        num_features = [int(embed_dim * 2 ** i) if int(embed_dim * 2 ** i)<320 else 320 for i in range(self.num_layers)]
...
class nnformer_v4d5_64(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_v4d5_64, self).__init__()
