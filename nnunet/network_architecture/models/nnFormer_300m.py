...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.reduce = nn.Conv3d(self.embed_dim, self.actual_dim, kernel_size=1)
        ...
        num_features = [int(actual_dim * 2 ** i) for i in range(self.num_layers)]
...
class nnformer_300m(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_300m, self).__init__()
        ...
        self.model_down = SwinTransformer(pretrain_img_size=pretrain_img_size, window_size=window_size,
                                          embed_dim=embed_dim, actual_dim=actual_dim,patch_size=patch_size, depths=depths,
                                          num_heads=num_heads, in_chans=input_channels)
        # for some reasons,the decoder is named with encoder
        self.encoder = encoder(pretrain_img_size=pretrain_img_size, embed_dim=actual_dim,
                               window_size=window_size,patch_size=patch_size, num_heads=num_heads[::-1][1:], depths=depths[::-1][1:])

        self.final = []
        for i in range(len(depths) - 1):
            self.final.append(final_patch_expanding(actual_dim * 2 ** i, num_classes, patch_size=patch_size))
