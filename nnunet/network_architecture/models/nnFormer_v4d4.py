...
class SingleConv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, Conv3D=nn.Conv3d, norm_layer=nn.InstanceNorm3d, act_layer=nn.GELU):
        super().__init__()
...
class OperationBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, kernel_size = 5, stride = 1, drop=0.,
                 act_layer=nn.GELU, norm_layer=nn.InstanceNorm3d):
        super().__init__()
        self.dim = dim
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.proj1 = project(in_chans, embed_dim // 2, [1, 2, 2], 1, nn.GELU, nn.InstanceNorm3d, False)
        self.proj2 = project(embed_dim // 2, embed_dim, [2, 2, 2], 1, nn.GELU, nn.InstanceNorm3d, True)
...
class nnformer_v4d4(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_v4d4, self).__init__()
