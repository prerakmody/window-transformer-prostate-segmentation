...
from nnunet.network_architecture.models.auxiliary_modules import DenseRelativeLoc
...
class nnformer_auxiliary(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True, sampling_method=None):

        super(nnformer_auxiliary, self).__init__()

        self._deep_supervision = deep_supervision
        self.do_ds = deep_supervision
        #For the test I just use the auxiliary task and set parmeters within the class
        #I use the simplest settings for my experiment
        # l1 loss with multi-scale
        self.use_drloc = True
        drloc_mode = 'l1'
        sample_size = 32
        use_abs = False
        ...
        # end of defining auxiliary tasks
        ...
        if self.use_drloc:
            ...
            for i_layer in range(len(depths)):
                self.drloc.append(DenseRelativeLoc(
                    ...
                    use_abs=use_abs))
            self.drloc_decoder = nn.ModuleList()
            for i_layer in range(len(depths)-1):
                self.drloc_decoder.append(DenseRelativeLoc(
                    in_dim=int(embed_dim * 2 ** (len(depths)-2-i_layer)),
                    out_dim=3,
                    sample_size=sample_size,
                    drloc_mode=drloc_mode,
                    use_abs=use_abs))
    ...
    def forward(self, x):
        ...
        for idx, feats in enumerate(skips):

            drloc_feats, deltaxyz = self.drloc[idx](feats,sampling_method = self.sampling_method)
            drloc_outs.drloc.append(drloc_feats)
            drloc_outs.deltaxy.append(deltaxyz)
            drloc_outs.plz.append(feats.shape[2])  # plane size = S = H = W

        neck = skips[-1]

        out = self.encoder(neck, skips)

        for idx, feats in enumerate(out):
            feats = feats.permute(0,4,1,2,3)
            drloc_feats, deltaxyz = self.drloc_decoder[idx](feats,sampling_method = self.sampling_method)
            drloc_outs.drloc.append(drloc_feats)
            drloc_outs.deltaxy.append(deltaxyz)
            drloc_outs.plz.append(feats.shape[2])  # plane size = S = H = W
        ...
        if self._deep_supervision and self.do_ds:
            return tuple([seg_outputs[-1]] + [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], seg_outputs[:-1][::-1])]), drloc_outs
        else:

            return seg_outputs[-1], drloc_outs
