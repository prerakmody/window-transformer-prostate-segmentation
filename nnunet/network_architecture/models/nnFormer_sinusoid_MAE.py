# nnformer without positional embedding
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.sinusoid:
            ...
            patches_resolution = [pretrain_img_size[0] // patch_size[0], pretrain_img_size[1] // patch_size[1],
                                  pretrain_img_size[2] // patch_size[2]]
            self.p_enc_3d = PositionalEncoding3D(embed_dim)
            self.sinusoid_pos_embed = nn.Parameter(torch.zeros(1, embed_dim, patches_resolution[0], patches_resolution[1], patches_resolution[2]),requires_grad=False)
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            sinusoid_pos_embed = self.p_enc_3d(self.sinusoid_pos_embed)
            sinusoid_pos_embed = F.interpolate(sinusoid_pos_embed, size=(Ws, Wh, Ww), align_corners=True,
                                               mode='trilinear')
            x = (x + sinusoid_pos_embed).flatten(2).transpose(1, 2)
        else:
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
...
class nnformer_sinusoid_mae(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(nnformer_sinusoid_mae, self).__init__()
