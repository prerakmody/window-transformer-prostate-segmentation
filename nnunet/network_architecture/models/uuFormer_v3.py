...
# Do not share parameter between global and local operator
# Depthwise convolution to get global feature map
class Mlp(nn.Module):
...
def window_partition(x, window_size, is_global=False):
    ...
    :param x: input feature map before window partition
    :param window_size: window_size
    :param is_global: used for global or local (deprecated when using depthwise-separable conv)
    :return:
    '''
    if is_global:
        B, S, H, W, C  = x.shape
...
class GlobalAttention(nn.Module):
    def __init__(self, dim, global_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        trunc_normal_(self.global_relative_position_bias_table, std=.02)
        self.softmax = nn.Softmax(dim=-1)
...
class GLTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, global_resolution, num_heads, window_size=7,
        ...
        self.global_attn = GlobalAttention(
            dim, global_size = global_resolution, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, global_x):
        ...
        # W-MSA
        attn_windows = self.local_attn(x_windows)
        ...
        global_x = self.global_attn(global_x)
...
class uuformer_v3(SegmentationNetwork):

    def __init__(self, input_channels, num_classes, deep_supervision=True):

        super(uuformer_v3, self).__init__()
