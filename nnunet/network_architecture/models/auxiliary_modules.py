...
def randn_sampling(maxint, sample_size, batch_size):
    return torch.randint(maxint, size=(batch_size, sample_size, 3))
def randn_sampling_neighbors(maxint, sample_size, batch_size, maxoffset):
    pxs = torch.randint(maxint, size=(batch_size, sample_size, 3))
    offset = torch.randint(low=-maxoffset, high=maxoffset+1, size=(batch_size, sample_size, 3))
    pys = pxs + offset
    pys = torch.clamp(pys, min=0, max=maxint-1)
    return pxs,pys
def randn_sampling_neighborwindow(maxint, sample_size, batch_size):
    pxs = torch.randint(maxint, size=(batch_size, sample_size, 3))
    windowIndex_pxs = pxs//4 #calculate window index for each pxs
    random_windowPos = torch.randint(4, size=(batch_size, sample_size, 3))
    pys = windowIndex_pxs*4 + random_windowPos
    return pxs, pys
...
def collect_samples_faster(feats, pxy, batch_size):
    n, c, s, h, w = feats.size()
    feats = feats.view(n, c, -1).permute(1, 0, 2).reshape(c, -1)  # [n, c, s, h, w] -> [n, c, shw] -> [c, nshw]
    pxy = ((torch.arange(n).long().to(pxy.device) * s * h * w).view(n, 1) + pxy[:, :, 0] * s * h + pxy[:, :, 1] * h + pxy[:, :, 2])
    pxy = pxy.view(-1)  # [n, m, 3] -> [nm]
...
class DenseRelativeLoc(nn.Module):
    ...
    def forward_features(self, x, mode="part", sampling_method=None):
        # x, feature map with shape: [B, C, H, W]
        B, C, S, H, W = x.size()

        if mode == "part":
            if sampling_method == 'neighbors':
                pxs, pys = randn_sampling_neighbors(H,self.sample_size,B,maxoffset=3)
                pxs = pxs.detach()
                pys = pys.detach()
            elif sampling_method == 'window_neighbors':
                pxs, pys = randn_sampling_neighborwindow(H,self.sample_size,B)
                pxs = pxs.detach()
                pys = pys.detach()
            else:
    ...
    def forward(self, x, normalize=True, sampling_method = None):
        pred_feats, deltaxy, H = self.forward_features(x, sampling_method = sampling_method)
        deltaxy = deltaxy.view(-1, 3)  # [B*sample_size, 2]
